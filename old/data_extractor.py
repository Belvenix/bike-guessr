...
def build_args() -> argparse.Namespace:
    ...
    data_to_download.add_argument('-n', '--netherlands', action='store_true')
...
def download_graph(place: str, tqdm_iterator: tqdm, target_dir: str):
    ...
    ox.save_graphml(merged_graph, filepath=f"./data/{target_dir}/{output}.xml")
...
if __name__ == "__main__":
    ...
    for place in place_iter:
        place_iter.set_description(
            f"# {place}")
        try:
            download_graph(place, place_iter, target_dir)
